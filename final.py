...
def main():
    ...
    with sync_playwright() as p:
        ...
        # 2. SCROLL PANJANG
        # ---------------------------------------------------------
        print("[INFO] Scrolling halaman reposts...")
        for i in range(25):
        ...
        # ---------------------------------------------------------
        # 3. AMBIL LIST REPOST DARI SELECTOR LAMA
        # ---------------------------------------------------------
        videos = page.locator("div[data-e2e='user-repost-item']")
        total = videos.count()

        print(f"[INFO] Total repost ditemukan: {total}")

        removed = 0

        # ---------------------------------------------------------
        # 4. MASUK VIDEO SATU PER SATU + REMOVE
        # ---------------------------------------------------------
        for i in range(total):
            ...
            video_url = page.url
            print(f"[INFO] URL Video: {video_url}")
            ...
            # -----------------------------------------------------
            # REMOVE REPOST DI SINI
            # -----------------------------------------------------
            try:
                remove_btn = page.locator("[aria-label='Remove repost']").first
                remove_btn.click()
                page.wait_for_timeout(1500)
                print("[OK] Repost berhasil dihapus.")
                removed += 1
            except:
                print("[SKIP] Tombol Remove repost tidak ditemukan.")

            # kembali ke list
            page.go_back()
            page.wait_for_timeout(2000)

        print("\n===== SELESAI REMOVE REPOST =====")
        print(f"[INFO] Total repost yang dihapus: {removed}")
